...
export default {
  ...
  methods: {
    ...
    setCount() {
      ...
      let currentGoods=carCookie.filter(function (item,index) {
        return item.id==that.$route.query.goodsId
      })
      if (currentGoods[0] != undefined) {
        console.log(currentGoods)
        this.goodsNum = currentGoods[0].num;
      }
    },
    // 收藏按钮点击事件
